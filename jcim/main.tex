...
We illustrate \emph{Speak to a Protein} on a complete drug-discovery workflow centered on cyclin-dependent kinase 2 (CDK2) \citep{Malumbres2009}, a validated yet contested oncology target with three decades of structural and bioactivity data. The case study tests whether the system can take an open-ended question about a target and, from a single user prompt, produce a coherent assessment that integrates structural data, bioactivity, binding-site analysis, and the relevant literature into a shareable report. The prompt issued to the agent is shown below; no further guidance was provided during the session.
...
From this single instruction the agent drafted its own research plan and executed it in four stages. It first resolved the canonical human CDK2 entry (UniProt P24941) and retrieved 498 linked PDB structures from RCSB. It then queried ChEMBL for 3{,}394 single-protein CDK2 activity records, together with matched datasets for CDK1, CDK4, CDK6, CDK7, CDK9, and the CDK2/cyclin~E1 and CDK2/cyclin~A2 complexes. From the structural collection it selected representatives spanning apo, cyclin-bound, orthosteric, allosteric, macrocyclic, and ternary-degrader states, loaded them into the 3D viewer, aligned them, and captured annotated snapshots of the cyclin-driven activation transition, the ATP-binding site, and the cyclin~E-dependent interface pocket. Finally, it pulled focused literature on CDK2 biology, cyclin-E dependency, allosteric chemistry, and degrader strategies, and compiled the full analysis into a self-contained LaTeX report reproduced verbatim in Appendix~S1.2.

The agent's main conclusions can be summarized as follows. CDK2 is presented as a structurally well-characterized target, with 489 X-ray and 9 cryo-EM entries at a median resolution of \SI{2.0}{\angstrom} and 374 cyclin complexes. Its ATP-binding site is described as broadly addressable, with a recurring contact set (Ile10, Ala31, Lys33, the Phe80--Leu83 hinge, Gln131--Asn132, Leu134, Asp145) that explains why subnanomolar potency is so often achieved. Selectivity is identified as the main challenge: across compounds with matched assays, the median CDK2 advantage over CDK1 is only 0.32 pChEMBL units, just 15.3\% reach a one-log margin, and the overlap against CDK9 is similar. To address this, the report points to two structural alternatives to the ATP site, namely the ANS allosteric site \citep{betzi2011allosteric} and the more recent cyclin~E-dependent interface pocket exemplified by PDB 8VQ3, and links them to a biomarker-led therapeutic case in CCNE1-amplified disease, cyclin-E-high tumours, and settings of CDK4/6-inhibitor resistance, supported by recent work on replication-stress biology \citep{brown2023forkrepair} and CDK2/cyclin~E heterobifunctional degraders \citep{kwiatkowski2025degrader}. The agent's overall recommendation is that CDK2 is a compelling target when biology, modality, and a selectivity strategy are aligned, but less attractive as a broad oncology kinase project.

Two figures from the agent's own report are reproduced in \cref{fig:cdk2_analysis}: an annotated 3D-viewer snapshot of the cyclin~E-dependent allosteric interface pocket, and a matched-compound selectivity panel for CDK2 against five related kinases. Together they show how the agent combined direct structural interpretation with quantitative analysis. The remaining figures, tables, and supporting data accompany the full report in the appendix.
...
\end{subfigure}
\caption{Figures the agent generated and embedded in its own report. \textbf{(a)} Annotated 3D-viewer snapshot of the cyclin~E-dependent allosteric interface pocket (PDB 8VQ3), jointly framed by CDK2 (green) and cyclin~E1 (blue) residues, captured by the agent during the analysis. \textbf{(b)} Matched-compound CDK2 selectivity relative to CDK1, CDK4, CDK6, CDK7, and CDK9 from filtered ChEMBL bioactivity; the dashed line is parity, the dotted red line marks a one-log margin in favour of CDK2.}
\label{fig:cdk2_analysis}
\end{figure}

Beyond the specific assessment of CDK2, the run illustrates the working pattern \emph{Speak to a Protein} is designed to enable. A single open-ended question produced a connected workflow that combined autonomous planning, multi-modal data integration, agentic use of the 3D viewer, and a publication-ready report within one session..

\section{Conclusions}\label{sec:discussion}

We presented \emph{Speak to a Protein}, a multimodal agentic co-scientist that turns protein analysis into an interactive dialogue grounded in language, code execution, and a live 3D viewer. The system retrieves and integrates information across literature, structural repositories, and bioactivity databases, drives the viewer with natural language, executes analysis code in a sandboxed environment, and uses agentic 3D vision to interpret the rendered scene. On BixBench-Verified-50, a benchmark of agentic bioinformatics and data-analysis tasks, the system reaches 78.5\% accuracy, on par with specialized bioinformatics agents and ahead of strong general-purpose coding agents. The two case studies illustrate the intended working pattern. The D3R interaction shows incremental, conversational use of the viewer, tabular data, and literature retrieval. The CDK2 study shows the system carrying a single open-ended drug-discovery question to a coherent target assessment, with structural snapshots, selectivity analysis, and a self-contained report produced in one session. Taken together, these results suggest that pairing a competent agent harness with structural biology tools meaningfully lowers the time and skill barrier between a scientific question and structured, reusable evidence, compressing workflows that traditionally take hours of manual data gathering and analysis into a single interactive session.
