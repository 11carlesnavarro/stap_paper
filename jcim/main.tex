...
The system architecture of \emph{Speak to a Protein} (\cref{fig:architecture}) is organized around four components: a frontend for user interaction and visualization, an agent harness that runs the agent loop and exposes tools to the language model, a search service that maintains semantic indices over codebases, documentation, and literature, and a sandboxed filesystem where code executes against a scientific Python runtime.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Figures/architecture.png}
\caption{\textbf{Overview of the system architecture.} The Frontend hosts a chat, a 3D molecular viewer, and an in-browser IPython console. The Agent Harness runs the agent loop, holds conversation memory, and calls tools. A search service indexes codebases, documentation, and literature. A sandboxed filesystem provides a per-project workspace and a scientific Python runtime, with restricted network egress.}\label{fig:architecture}
\end{figure}

The frontend incorporates a conversational chat panel and a 3D molecular viewer (\cref{fig:viewer} and \cref{subsec:viewer}). Alongside these, an in-browser IPython console powered by Pyodide allows users to inspect or extend any analysis directly in the browser, with no installation. The frontend renders the agent's responses, displaying text in chat and applying viewer commands as the agent issues them.

The agent harness runs an agent loop that interleaves model reasoning with tool calls and maintains a per-session conversational memory, allowing the assistant to plan multi-step workflows and revisit earlier results. The underlying language model can run via an external API or as a self-hosted local model, with the choice made at configuration time. The harness exposes a small, general-purpose tool set:
